...
def open_filter_and_search(driver, filter_name):
    ...
    try:
        ...
        WebDriverWait(driver, 30).until(EC.presence_of_element_located(
            (By.XPATH, constant.FILTER_DICT_WITH_EXPAND[filter_types.FILTER_TYPES[filter_name]]))).click()
        print('filter type clicked')
        time.sleep(7)
    except Exception as e:
        print('except block')
        WebDriverWait(driver, 30).until(EC.presence_of_element_located(
            (By.XPATH, constant.FILTER_DICT_WITHOUT_EXPAND[filter_types.FILTER_TYPES[filter_name]]))).click()
        print('filter type clicked')
        time.sleep(7)
